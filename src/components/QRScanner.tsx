...
import { ArrowRight, Camera, Loader2, ShieldAlert, X, Zap } from "lucide-react";
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-black/98 backdrop-blur-2xl">
      <div className="flex items-center justify-between border-b border-white/5 bg-black/40 p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="leading-none text-xl font-black uppercase tracking-tighter text-white italic">{title}</h2>
      ...
      <div className="relative flex flex-1 flex-col items-center justify-center p-8">
        <div className="relative aspect-square w-full max-w-[320px] overflow-hidden rounded-[3rem] border-2 border-white/10 bg-black shadow-2xl">
        ...
        <div className="mt-10 flex w-full max-w-sm flex-col items-center gap-6">
          ...
          <p className="text-center text-[10px] font-bold uppercase tracking-widest text-white/30">{demoMode ? "Modo demo ativo: use o botao acima para simular a leitura." : scanHint}</p>
        </div>
      </div>
    </div>
  );
