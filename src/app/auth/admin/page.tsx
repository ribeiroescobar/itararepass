...
import { ShieldCheck, Mail, Lock, Loader2, ArrowLeft, Key, Zap, UserPlus } from "lucide-react";
...
export default function AdminAuthPage() {
  ...
  const handleAuth = async (e: React.FormEvent, mode: 'login' | 'register') => {
    ...
    if (mode === 'login') {
      ...
      register({ email, pass: password, name: name || "Douglas", role: 'prefeitura', additional: {} }, () => setIsActionLoading(false));
    }
  };
          ...
          <Tabs defaultValue="login" className="w-full">
            ...
            <TabsContent value="login" className="p-6 pt-2 space-y-6">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-[9px] font-black uppercase"><span className="bg-[#16261e] px-4 text-white/20">Demonstração</span></div>
              </div>
              <Button onClick={() => login("gestor@itarare.gov.br", "itarare2025", () => setIsActionLoading(false))} variant="outline" className="w-full border-blue-500/20 bg-blue-500/10 hover:bg-blue-500/20 rounded-2xl h-12 text-[10px] font-black uppercase text-blue-400">
                <Zap className="w-3 h-3 mr-2 fill-blue-400" /> Acesso Rápido Demo
              </Button>
            </TabsContent>
            ...
            <TabsContent value="register" className="p-6 pt-2 space-y-6">
              <form onSubmit={(e) => handleAuth(e, 'register')} className="space-y-4">
                <div className="space-y-2">
                  <Label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-1">Nome (Douglas ou Funcionário)</Label>
                  <Input required placeholder="Ex: Douglas Escobar" value={name} onChange={(e) => setName(e.target.value)} className="bg-black/40 border-white/5 rounded-2xl h-14 px-5 text-white" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-1">{t('label_institutional_email')}</Label>
                  <Input type="email" required placeholder="douglasescobarribeiro@gmail.com" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-black/40 border-white/5 rounded-2xl h-14 px-5 text-white" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-1">Defina uma Senha</Label>
                ...
                <Button disabled={isActionLoading || isUserLoading} type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black h-14 rounded-2xl text-xs uppercase tracking-widest">
                  {isActionLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><UserPlus className="w-4 h-4 mr-2" /> Criar Minha Conta Master</>}
                </Button>
              </form>
            </TabsContent>
          </Tabs>
