      ...
      <div className="absolute top-6 left-6 flex items-center gap-3 z-50">
        <div className="flex items-center gap-1 bg-black/60 p-1 rounded-xl border border-white/5 h-11 shadow-2xl">
          <button
            ...
            className={`px-2 h-9 rounded-lg transition-all flex items-center gap-2 ${language === "pt" ? "bg-white/10 ring-1 ring-white/20" : "opacity-40"}`}
          >
            <FlagBR />
          ...
          <button
            ...
            className={`px-2 h-9 rounded-lg transition-all flex items-center gap-2 ${language === "en" ? "bg-white/10 ring-1 ring-white/20" : "opacity-40"}`}
          >
            <FlagUS />
        ...
        <Card className="bg-white/5 border-blue-500/20 p-2 rounded-[3rem] shadow-2xl backdrop-blur-xl">
          <Tabs defaultValue="login" className="w-full">
            <TabsList className="grid grid-cols-2 bg-black/40 rounded-[2rem] m-2 h-14">
              <TabsTrigger value="login" className="rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest">
                {t("login_tab")}
              </TabsTrigger>
              <TabsTrigger value="register" className="rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest">
                {t("register_tab")}
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="login" className="p-6 pt-2 space-y-6">
              <form onSubmit={(e) => handleAuth(e, "login")} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      type="email"
                      required
                      placeholder="ex: nome@itarare.gov.br"
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      className="bg-black/40 border-white/5 rounded-2xl h-14 pl-12 text-white"
                    />
                  </div>
                </div>
                <div className="space-y-2">
                ...
                <Button
                  ...
                  type="submit"
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black h-14 rounded-2xl text-xs uppercase tracking-widest"
                >
              ...
              <form onSubmit={(e) => handleAuth(e, "register")} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="bg-black/40 border-white/5 rounded-2xl h-14 px-5 text-white"
                  />
                </div>
                <div className="space-y-2">
                  <Label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-1">
                  ...
                  <Input
                    type="email"
                    required
                    placeholder="contato@itarare.gov.br"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className="bg-black/40 border-white/5 rounded-2xl h-14 px-5 text-white"
                  />
                ...
                >
                  {isActionLoading ? (
                    ...
                    <>
                      <UserPlus className="w-4 h-4 mr-2" /> {registerLabel}
                    </>
                  )}
                </Button>
              </form>
